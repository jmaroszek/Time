...
# One log, both streams, in the order they happened.
#
# This is harder than it looks and two obvious approaches fail. Start-Process
# refuses to aim both of its redirects at a single file. And PowerShell's own
# `2>&1` merges only its error stream and the native commands it invokes
# directly -- publish_release.ps1 runs `npm run tauri build`, whose stderr is
# written by a grandchild straight to the inherited OS handle, so PowerShell
# never sees it. Merging at the PowerShell level does not merely fail to capture
# cargo, it discards it.
#
# cmd's `> file 2>&1` duplicates the handles for the whole process subtree, which
# is what actually works. It matters because cargo and the Tauri bundler emit
# "Compiling", "Finished release", "Built application at" and "Finished 1 bundle
# at" on stderr -- the entire Rust and NSIS phase. For the phantom Ctrl+C, whose
# cause is still unknown, knowing what was running when it landed is the whole
# question, and that needs one file in the real order.
#
# The command is written to a runner script rather than inlined so that cmd's
# parser only ever sees two quoted paths. Release notes are free text and have
# already broken argument passing once; they stay inside the runner as a
# PowerShell literal, where quoting is well defined.
$publishScript = Join-Path $PSScriptRoot "publish_release.ps1"
$runner = Join-Path $logDir "release-$stamp.run.ps1"

# Splatted rather than written as a command line, so an optional parameter is a
# hashtable entry that is present or absent, with no line-continuation or
# conditional-comma handling to get wrong.
$runnerBody = @(
    "# Generated by release.ps1. Kept beside the log as an exact record of what",
    "# this build was asked to do.",
    "`$ErrorActionPreference = 'Stop'",
    "`$arguments = @{",
    "    DownloadBaseUrl = $(ConvertTo-PowerShellLiteral $DownloadBaseUrl)"
)
if ($Notes) {
    $runnerBody += "    Notes = $(ConvertTo-PowerShellLiteral $Notes)"
}
$runnerBody += @(
    "}",
    "& $(ConvertTo-PowerShellLiteral $publishScript) @arguments",
    "exit `$LASTEXITCODE"
)
...
$publishArgs = @(
    "/c",
    "pwsh -NoProfile -NoLogo -NonInteractive -File $(ConvertTo-ProcessArgument $runner)" +
    " > $(ConvertTo-ProcessArgument $log) 2>&1"
)
...
# cmd, not pwsh: the redirection above is cmd's, and it is what merges the
# streams for every descendant. Hidden window keeps the isolation -- the child is
# not attached to this console, so a control event here cannot reach it.
$build = Start-Process cmd -ArgumentList $publishArgs -WindowStyle Hidden -PassThru
