...
function DeltaBadge({ app }: { app: AppDelta }) {
  ...
  return (
    <span
      className={`inline-grid w-14 shrink-0 grid-cols-[12px_44px] items-baseline text-[11px] font-normal tabular-nums ${cls}`}
    ...
    >
      <span className="text-center" aria-hidden="true">{sign}</span>
      <span className="text-right" aria-hidden="true">{Math.abs(pct)}%</span>
    </span>
  );
}
